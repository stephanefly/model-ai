...
def detect_main_catalogue_code(request: dict) -> str | None:
    """Reconnaît le pack principal pour appliquer aussi son prix plancher."""
    explicit_code = request.get("catalogue_code")
    if explicit_code:
        return str(explicit_code)

    if _is_selected(request, "product_miroirbooth") and _is_selected(
...
def calculate_catalogue_floor(request: dict, catalogue: pd.DataFrame) -> float:
    """Retourne le plancher commercial du catalogue quand il est renseigné."""
    code = detect_main_catalogue_code(request)
    if not code or code not in catalogue.index:
        return 0.0

    floor_price = catalogue.loc[code, "floor_price_eur"]
    if pd.isna(floor_price):
        return 0.0
...
def apply_candidate_price(request: dict, gross: float, price: float) -> dict:
    """Met à jour le prix et les remises comme dans les données d'entraînement."""
    candidate = dict(request)
    candidate["price_catalog_gross"] = gross
    candidate["price_proposed"] = price

    total_discount = max(gross - price, 0.0)
    option_discount = min(
        float(request.get("price_option_discount") or 0),
        total_discount,
    )
    remaining_discount = total_discount - option_discount

    if request.get("price_product_discount") is None:
        # Pour un pack de plusieurs prestations, la remise est principalement
        # une remise produit.
        product_discount = remaining_discount
    else:
        product_discount = min(
            float(request.get("price_product_discount") or 0),
            remaining_discount,
        )

    global_discount = max(
        total_discount - option_discount - product_discount,
        0.0,
    )

    candidate["price_option_discount"] = option_discount
    candidate["price_product_discount"] = product_discount
    candidate["price_global_discount"] = global_discount
    candidate["price_total_declared_discount"] = total_discount
...
def optimize(request: dict, bundle: dict, catalogue: pd.DataFrame, config: dict) -> dict:
    ...
    catalogue_floor = calculate_catalogue_floor(request, catalogue)
    ...
    for price in prices:
        candidate = apply_candidate_price(
            request,
            gross,
            price,
        )
    ...
    if feasible:
        ...
        # Si aucun prix ne respecte le seuil de signature, on ne maximise plus
        # la marge attendue. On retient le prix avec la meilleure probabilité
        # comme solution de secours et on interdit toute utilisation automatique.
        selected = max(
            selection_rows,
            key=lambda row: (
                row["signature_probability"],
                -row["price_eur"],
            ),
        )
    ...
    return {
        ...
        "catalogue_floor_price_eur": catalogue_floor,
